    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= 5; pageNum++) {
        try {
          ...
          console.log(`[Voltum Титан] Загружаем страницу ${pageNum}...`);
          ...
          if (response.data && response.data.products && Array.isArray(response.data.products)) {
            ...
            console.log(`[Voltum Титан] Страница ${pageNum}: +${pageProducts.length} товаров (всего: ${allProducts.length})`);
            
            if (pageProducts.length < params.limit) {
              console.log(`[Voltum Титан] Последняя страница: ${pageNum}`);
              break;
            }
          } else {
            console.log(`[Voltum Титан] Страница ${pageNum}: нет данных`);
            break;
          }
          
          await new Promise(resolve => setTimeout(resolve, 100));
        } catch (error) {
          console.error(`[Voltum Титан] Ошибка страницы ${pageNum}:`, error);
          break;
        }
      }
      ...
      if (uniqueProducts.length > 0) {
        const filteredProducts = uniqueProducts.filter((product: any) => {
          ...
          const isTitan = category.toLowerCase().includes('титан') || 
                         name.toLowerCase().includes('титан');
          
          console.log(`[Voltum Титан] "${name}" | Категория: "${category}" | Титан: ${isTitan}`);
        ...
        console.log('Товары Voltum Титан:', filteredProducts.length);
      }
    } catch (error) {
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          {totalProducts > 0 ? (
            ...
            <p className="text-gray-400">Электроустановочные изделия Voltum цвета Титан</p>
          )}
        </div>
