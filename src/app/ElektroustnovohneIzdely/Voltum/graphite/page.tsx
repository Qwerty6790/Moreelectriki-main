    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= 5; pageNum++) {
        try {
          ...
          console.log(`[Voltum Графит] Загружаем страницу ${pageNum}...`);
          ...
          if (response.data && response.data.products && Array.isArray(response.data.products)) {
            ...
            console.log(`[Voltum Графит] Страница ${pageNum}: +${pageProducts.length} товаров (всего: ${allProducts.length})`);
            
            if (pageProducts.length < params.limit) {
              console.log(`[Voltum Графит] Последняя страница: ${pageNum}`);
              break;
            }
          } else {
            console.log(`[Voltum Графит] Страница ${pageNum}: нет данных`);
            break;
          }
          
          await new Promise(resolve => setTimeout(resolve, 100));
        } catch (error) {
          console.error(`[Voltum Графит] Ошибка страницы ${pageNum}:`, error);
          break;
        }
      }
      ...
      if (uniqueProducts.length > 0) {
        const filteredProducts = uniqueProducts.filter((product: any) => {
          ...
          const isGraphite = category.toLowerCase().includes('графит') || 
                            name.toLowerCase().includes('графит');
          
          console.log(`[Voltum Графит] "${name}" | Категория: "${category}" | Графит: ${isGraphite}`);
        ...
        console.log('Товары Voltum Графит:', filteredProducts.length);
      }
    } catch (error) {
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h2 className="text-5xl font-bold text-white mb-5">Графит</h2>
          {totalProducts > 0 ? (
            <p className="text-white">Найдено {totalProducts} {totalProducts === 1 ? 'товар' : totalProducts < 5 ? 'товара' : 'товаров'}</p>
          ) : !loading && (
            <p className="text-gray-400">Электроустановочные изделия Voltum цвета Графит</p>
          )}
        </div>
