    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= 5; pageNum++) {
        try {
          ...
          console.log(`[Voltum Сталь] Загружаем страницу ${pageNum}...`);
          ...
          if (response.data && response.data.products && Array.isArray(response.data.products)) {
            ...
            console.log(`[Voltum Сталь] Страница ${pageNum}: +${pageProducts.length} товаров (всего: ${allProducts.length})`);
            
            if (pageProducts.length < params.limit) {
              console.log(`[Voltum Сталь] Последняя страница: ${pageNum}`);
              break;
            }
          } else {
            console.log(`[Voltum Сталь] Страница ${pageNum}: нет данных`);
            break;
          }
          
          await new Promise(resolve => setTimeout(resolve, 100));
        } catch (error) {
          console.error(`[Voltum Сталь] Ошибка страницы ${pageNum}:`, error);
          break;
        }
      }
      ...
      if (uniqueProducts.length > 0) {
        const filteredProducts = uniqueProducts.filter((product: any) => {
          ...
          const isSteel = category.toLowerCase().includes('сталь') || 
                         name.toLowerCase().includes('сталь');
          
          console.log(`[Voltum Сталь] "${name}" | Категория: "${category}" | Сталь: ${isSteel}`);
        ...
        console.log('Товары Voltum Сталь:', filteredProducts.length);
      }
    } catch (error) {
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          {totalProducts > 0 ? (
            ...
            <p className="text-gray-400">Электроустановочные изделия Voltum цвета Сталь</p>
          )}
        </div>
