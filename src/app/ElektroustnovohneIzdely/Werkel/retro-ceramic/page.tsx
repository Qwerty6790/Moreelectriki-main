    ...
    try {
      ...
      if (data && data.products) {
        ...
        const filteredProducts = data.products.filter((product: any) => {
          ...
          // Проверяем что это серия Ретро и керамическое
          if (!name.includes('Ретро')) return false;
          if (!name.includes('керамическое')) return false;
    ...
    <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh', color: 'var(--foreground)' }}>
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 p-48">
        <nav className="flex items-center space-x-2 text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-white">Retro - Керамическое</span>
        </nav>
      </div>

      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 ">
        <div className="mb-8">
          <h2 className="text-5xl font-bold text-white mb-5">Retro - Керамическое</h2>
          {totalProducts > 0 ? (
